...
describe("time", () => {
  ...
  it("should throw error when time is less than zero", () => {
    expect.assertions(4);
    expect(() => {
      secondsFromNow(0, errorPath);
    }).toThrow();
    expect(() => {
      minutesFromNow(-1, errorPath);
    }).toThrow();
    expect(() => {
      hoursFromNow(-80, errorPath);
    }).toThrow();
    expect(() => {
      daysFromNow(8.25, errorPath);
    }).toThrow();
  });
});
